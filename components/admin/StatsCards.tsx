...
export function StatsCards({ registrations }: { registrations: Registration[] }) {
  ...
  const cards = [
    ...
    },
    {
      ...
      progressClass: "bg-linear-to-r from-kinetic-primary-container to-kinetic-primary-fixed shadow-[0_0_10px_rgba(211,237,134,0.8)]",
      active: true
    },
    {
      label: "Remaining",
        ...
        return (
          <div
            ...
            className={`admin-glass-card relative flex flex-col justify-between overflow-hidden rounded-xl p-4 md:p-6 ${
              card.active ? "admin-glow-active border-kinetic-primary-container/50" : ""
          ...
          >
            ...
            <div className="absolute right-0 top-0 p-3 opacity-20">
              <Icon className="size-10 text-kinetic-surface-bright md:size-[60px]" aria-hidden="true" />
            </div>
            <div className="relative z-10">
              <p
                className={`mb-1 text-xs font-bold uppercase ${
                  card.active ? "flex items-center gap-2 text-kinetic-primary-container" : "text-kinetic-on-surface-variant"
                }`}
              >
                {card.label}
                {card.active ? (
                  <span className="size-2 animate-pulse rounded-full bg-kinetic-primary-container" />
                ) : null}
              </p>
              <div className="flex items-baseline gap-2">
                ...
                {"suffix" in card && card.suffix ? (
                  <span className="text-sm font-bold text-kinetic-primary-container/80">{card.suffix}</span>
                ) : null}
              </div>
            </div>
